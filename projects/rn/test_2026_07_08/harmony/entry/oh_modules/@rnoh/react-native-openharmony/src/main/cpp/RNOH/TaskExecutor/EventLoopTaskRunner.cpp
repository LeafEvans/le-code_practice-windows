...
EventLoopTaskRunner::EventLoopTaskRunner(
    ...
    : m_name(name),
      m_loop(loop),
      m_isAlive(std::make_shared<std::atomic_bool>(true)),
      m_asyncHandle(std::make_unique<uv::Async>(
          m_loop,
          [this, isAlive = m_isAlive] {
            if (!isAlive->load(std::memory_order_acquire)) {
              return;
            }
            this->executeTask();
          })),
...
EventLoopTaskRunner::~EventLoopTaskRunner() {
  ...
  if (m_executeTaskInFlight.load(std::memory_order_acquire) > 0) {
    std::unique_lock<std::mutex> lock(m_executeTaskMutex);
    m_executeTaskCv.wait(lock, [&] {
      return m_executeTaskInFlight.load(std::memory_order_acquire) == 0;
    });
  }
  RNOH_ASSERT_MSG(
      cleanedUp,
...
void EventLoopTaskRunner::cancelDelayedTask(DelayedTaskId taskId) {
  ...
  if (isOnCurrentThread()) {
    ...
    runAsyncTask(std::move(removeTask));
  }
}
...
void EventLoopTaskRunner::executeTask() {
  struct ExecuteTaskInFlightGuard {
    EventLoopTaskRunner* self;
    explicit ExecuteTaskInFlightGuard(EventLoopTaskRunner* s) : self(s) {
      self->m_executeTaskInFlight.fetch_add(1, std::memory_order_acq_rel);
    }
    ~ExecuteTaskInFlightGuard() {
      if (self->m_executeTaskInFlight.fetch_sub(1, std::memory_order_acq_rel) ==
          1) {
        std::lock_guard<std::mutex> lock(self->m_executeTaskMutex);
        self->m_executeTaskCv.notify_all();
      }
    }
  };
  ExecuteTaskInFlightGuard inFlightGuard(this);

  Task task = popNextTask();
  if (task) {
    try {
      facebook::react::TraceSection s("#RNOH::TaskRunner::task");
      task();
...
void EventLoopTaskRunner::cleanup() {
  ...
  runSyncTask([this] {
    std::lock_guard<std::mutex> queueLock(m_mutex);
    // Invalidate callback only when cleanup task is actually running
    // on the owning loop thread; otherwise runSyncTask may wait forever.
