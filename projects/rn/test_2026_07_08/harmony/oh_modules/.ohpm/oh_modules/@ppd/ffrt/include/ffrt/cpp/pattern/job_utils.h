...
/**
 * @struct capbility
 * @brief ffrt so capbility helper.
 */
struct capbility {
    struct handle {
        handle()
    ...
    static inline auto ffrt_fiber_init_f = reinterpret_cast<decltype(&ffrt_fiber_init)>(capbility::get_func("ffrt_fiber_init"));
    static inline auto ffrt_fiber_switch_f = reinterpret_cast<decltype(&ffrt_fiber_switch)>(capbility::get_func("ffrt_fiber_switch"));
#endif

	/**
	 * @brief check libffrt.so in rom has fiber feature
	 *
	 * @return return true has fiber feature, otherwise false.
	 */
...
struct fiber : detail::non_copyable {
    ...
    static fiber* init(std::function<void()>&& f, void* stack, size_t stack_size)
    {
        ...
        if FFRT_API_UNLIKELY(!capbility::ffrt_fiber_init_f || capbility::ffrt_fiber_init_f(&c->fb, reinterpret_cast<void(*)(void*)>(fiber_entry), c,
    ...
    bool start()
    {
        ...
        do {
            ...
            // Irrecoverable error: capbility::ffrt_fiber_switch_f is null.
            // Before invoking job_partner::submit_to_master(), ensure that libffrt.so supports fiber features
            // by checking ffrt::capbility::support_fiber_feature().
            capbility::ffrt_fiber_switch_f(&link, &fb);
    ...
    static inline void suspend(thread_env& e, bool (*cond)(void*) = nullptr)
    {
        ...
        // Before invoking job_partner::submit_to_master(), ensure that libffrt.so supports fiber features
        // by checking ffrt::capbility::support_fiber_feature().
        capbility::ffrt_fiber_switch_f(&j->fb, &j->link);
    }
