...
std::shared_ptr<RNInstanceInternal> createRNInstance(
    ...
    RNOHMarker::RNOHMarkerListener::Shared markerListener,
  ...
  // ContextContainer lookups are type-specific, so store the shared manager as
  // TextLayoutManager for getManagerByName(TextLayoutManager).
  std::shared_ptr<facebook::react::TextLayoutManager> textLayoutManager =
      std::make_shared<TextMeasurer>(
          contextContainer, featureFlagRegistry, fontRegistry, id);
  auto shadowViewRegistry = std::make_shared<ShadowViewRegistry>();
  contextContainer->insert("TextLayoutManager", textLayoutManager);
  ...
  auto rnInstance = std::make_shared<RNInstanceCAPI>(
      ...
      hspModuleName);
  rnInstance->onCreate();
  // Configure modal content shrink feature
  auto enableModalContentShrink =
      featureFlagRegistry->isFeatureFlagOn("ENABLE_MODAL_CONTENT_SHRINK");
  facebook::react::GuideLayout::getInstance().setModalContentShrinkEnabled(
      enableModalContentShrink);
  // Configure background GC feature
  auto enableBackgroundGC =
      featureFlagRegistry->isFeatureFlagOn("ENABLE_BACKGROUND_GC");
  rnInstance->setEnableBackgroundGC(enableBackgroundGC);
