...
static napi_value onInit(napi_env env, napi_callback_info info) {
  ...
  return invoke(env, [&] {
    ...
    static std::once_flag markerListenersOnceFlag{};
    std::call_once(markerListenersOnceFlag, []() {
      static auto performanceMetricsListener =
          std::shared_ptr<PerformanceMetricsRegistry>(
              &PerformanceMetricsRegistry::getInstance(),
              [](PerformanceMetricsRegistry*) {});
      RNOHMarker::addListener(performanceMetricsListener);
#ifdef WITH_HITRACE_REACT_MARKER
      static auto hiTraceListener = std::shared_ptr<HiTraceRNOHMarkerListener>(
          &HiTraceRNOHMarkerListener::getInstance(),
          [](HiTraceRNOHMarkerListener*) {});
      RNOHMarker::addListener(hiTraceListener);
#endif
    });
...
static napi_value onCreateRNInstance(napi_env env, napi_callback_info info) {
  return invoke(env, [&] {
    ...
    RNOHMarker::setAppStartTime(
        HighResTimeStamp::now().toDOMHighResTimeStamp());
    ...
    auto markerListener =
        std::make_shared<RNInstanceInternal::RNInstanceRNOHMarkerListener>(
            arkTSChannel);
    RNOHMarker::addListener(markerListener);
...
static napi_value setSurfaceProps(napi_env env, napi_callback_info info) {
  return invoke(env, [&] {
    ...
    facebook::react::Tag surfaceId = arkJS.getDouble(args[1]);
    auto surface =
        rnInstance->getSurfaceByRootTag(surfaceId); // rootTag == surfaceId
    surface->lock()->setProps(arkJS.getDynamic(args[2]));
    return arkJS.getNull();
  });
}
...
static napi_value initializePerformanceMetrics(
    napi_env env,
    napi_callback_info info) {
  return invoke(env, [&] {
    ArkJS arkJS(env);
    auto args = arkJS.getCallbackArgs(info, 1);
    size_t rnInstanceId = arkJS.getDouble(args[0]);
    PerformanceMetricsRegistry::getInstance().addNewRNInstanceMetrics(
        rnInstanceId);
    return arkJS.getNull();
  });
}
