...
export type CppFeatureFlag = "PARTIAL_SYNC_OF_DESCRIPTOR_REGISTRY" | "WORKER_THREAD_ENABLED" | "ENABLE_MODAL_CONTENT_SHRINK" | "ENABLE_BACKGROUND_GC"
...
export class NapiBridge {
  ...
  initializePerformanceMetrics(rnInstanceId: number) {
    const result = this.libRNOHApp?.initializePerformanceMetrics(rnInstanceId)
    return this.unwrapResult(result)
  }
  ...
  getPerformanceMetricsSnapshot(rnInstanceId: number): RNInstancePerformanceMetricsSnapshot | undefined {
