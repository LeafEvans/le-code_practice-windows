...
export class SafeAreaTurboModule extends UITurboModule {
  ...
  static async create(ctx: UITurboModuleContext, statusBarTurboModule: StatusBarTurboModule) {
    await ctx.safeAreaInsetsProvider.ready;
  ...
  constructor(ctx: UITurboModuleContext, statusBarTurboModule: StatusBarTurboModule) {
    ...
    this.postStatusBarHeight(this.initialInsets.top);
  ...
  private onSafeAreaChange(insets: SafeAreaInsets) {
    ...
    this.postStatusBarHeight(insets.top);
  }

  public sendCurrentStatusBarHeight() {
    this.postStatusBarHeight(this.ctx.safeAreaInsetsProvider.safeAreaInsets.top);
  }

  private postStatusBarHeight(top: number) {
    this.ctx.rnInstance.postMessageToCpp('STATUS_BAR_HEIGHT', { statusBarHeight: top });
  }
