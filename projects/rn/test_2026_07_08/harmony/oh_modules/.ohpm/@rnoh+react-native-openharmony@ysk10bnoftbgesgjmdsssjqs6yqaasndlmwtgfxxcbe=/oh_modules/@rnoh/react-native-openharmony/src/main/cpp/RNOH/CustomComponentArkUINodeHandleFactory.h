 ...
 public:
  ...
  std::optional<CustomComponentArkUINodeHandle> create(
    ...
    return CustomComponentArkUINodeHandle{
        ...
        contentHandle,
        [env = m_env,
         disposeRef = std::move(n_disposeRef),
         delegateGetterRef = std::move(n_delegateGetterRef)] {
          ArkJS arkJS(env);
          auto disposeCallback = arkJS.getReferenceValue(disposeRef);
          arkJS.call(disposeCallback, {});
          arkJS.deleteReference(delegateGetterRef);
        },
        [env = m_env, delegateGetterRef = std::move(n_delegateGetterRef)]()
            -> std::optional<std::shared_ptr<RNArkTSComponentDelegate>> {
          ArkJS arkJS(env);
