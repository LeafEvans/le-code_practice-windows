...
thread_local std::deque<HostObjectProxy::GetterContext>
    HostObjectProxy::getterStack;
...
JSVM_Value HostObjectProxy::Enumerator(
  ...
  while (true) {
    ...
    if (OH_JSVM_GetPrototype(env, currentThis, &prototype) != JSVM_OK) {
      OH_JSVM_ThrowError(
          env, nullptr, "HostObjectProxy.Enumerator: Failed to get prototype");
      return JSVMConverter::JsiToJSVM(env, Value::undefined());
    }
    ...
    if (OH_JSVM_StrictEquals(env, currentThis, prototype, &isSameObject) !=
      ...
      OH_JSVM_ThrowError(
          env,
          nullptr,
          "HostObjectProxy.Enumerator: StrictEquals check failed");
      return JSVMConverter::JsiToJSVM(env, Value::undefined());
    }
    if (isSameObject) {
      OH_JSVM_ThrowError(
          env,
          nullptr,
          "HostObjectProxy.Enumerator: Property not found in prototype chain");
      return JSVMConverter::JsiToJSVM(env, Value::undefined());
    }
    currentThis = prototype;
  }
  ...
  try {
    ...
    std::string exceptionString("Exception in HostObjectProxy.Enumerator: ");
    exceptionString += ex.what();
    OH_JSVM_ThrowError(env, nullptr, exceptionString.c_str());
    return JSVMConverter::JsiToJSVM(env, Value::undefined());
  } catch (...) {
    OH_JSVM_ThrowError(
        env, nullptr, "Exception in HostObjectProxy.Enumerator: <unknown>");
    return JSVMConverter::JsiToJSVM(env, Value::undefined());
  }
