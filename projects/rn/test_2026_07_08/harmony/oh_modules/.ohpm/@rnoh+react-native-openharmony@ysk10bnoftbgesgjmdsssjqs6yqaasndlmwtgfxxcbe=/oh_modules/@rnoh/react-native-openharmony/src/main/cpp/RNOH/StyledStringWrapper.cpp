...
float StyledStringWrapper::resolveFontMultiplier(
    const facebook::react::TextAttributes& textAttributes) const {
  const bool hasAllowFontScaling = textAttributes.allowFontScaling.has_value();
  const bool allowFontScaling =
      hasAllowFontScaling ? textAttributes.allowFontScaling.value() : true;
  const bool hasMaxFontSizeMultiplier =
      !isnan(textAttributes.maxFontSizeMultiplier);

  if (!hasAllowFontScaling && !hasMaxFontSizeMultiplier) {
    return m_defaultFontMultiplier;
  }
  ...
  const float maxMultiplier = textAttributes.maxFontSizeMultiplier;
  if (!hasMaxFontSizeMultiplier || maxMultiplier < 1.0f) {
    return m_fontScale;
  }

  return std::min(m_fontScale, maxMultiplier);
}
void StyledStringWrapper::addTextFragment(
  ...
  const float fontMultiplier = resolveFontMultiplier(fragment.textAttributes);
  OH_Drawing_SetTextStyleFontSize(
      textStyle.get(), fontSize * m_scale * fontMultiplier);
  // halfLeading
