...
JSVMRuntime::~JSVMRuntime() {
  ...
  // NOTE:
  // JSVM environment destruction will clean up VM-side references for us.
  // At this point we must not call any JSVM APIs using `env` anymore.
  // We only need to delete the C++ wrapper nodes and unlink the list.
...
inline Runtime::PointerValue* JSVMRuntime::clone(
    const Runtime::PointerValue* pv) {
  return reinterpret_cast<JSVMPointerValue*>(
...
void JSVMRuntime::setNativeState(
  ...
  DLOG(INFO) << __func__;
  JSVMUtil::HandleScopeWrapper scope(env);
  ObjectNativeState* objectNativeState = new ObjectNativeState(state);
...
Value JSVMRuntime::lockWeakObject(const WeakObject& weakObj) {
  DLOG(INFO) << __func__;
  JSVMUtil::HandleScopeWrapper scope(env);
  JSVM_Value val = static_cast<const JSVMPointerValue*>(
                       JSVMRuntime::getPointerValue(weakObj))
                       ->GetValue();
  if (val == nullptr) {
...
void JSVMRuntime::notifyMemoryPressure(JSVM_MemoryPressureLevel level) {
  if (unlikely(env == nullptr)) {
    LOG(WARNING) << "JSVMRuntime::notifyMemoryPressure called with null env";
    return;
  }
