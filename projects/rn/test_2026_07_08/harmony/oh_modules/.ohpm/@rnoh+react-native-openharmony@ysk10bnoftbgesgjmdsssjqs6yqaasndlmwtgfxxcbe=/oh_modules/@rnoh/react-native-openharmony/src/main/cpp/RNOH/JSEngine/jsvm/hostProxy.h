...
class HostObjectProxy : public HostProxy {
 ...
 private:
  ...
  static thread_local std::deque<GetterContext> getterStack;
...
class ObjectNativeState : public HostProxy {
 public:
  explicit ObjectNativeState(std::shared_ptr<NativeState> state)
      : HostProxy(ProxyType::T_NATIVE_STATE), nativeState(state) {}
