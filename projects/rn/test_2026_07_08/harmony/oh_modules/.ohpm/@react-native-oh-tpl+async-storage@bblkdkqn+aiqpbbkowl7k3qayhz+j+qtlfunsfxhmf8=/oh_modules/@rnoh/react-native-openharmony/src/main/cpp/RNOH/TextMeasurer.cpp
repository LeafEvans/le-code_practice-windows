...
facebook::react::TextMeasurement TextMeasurer::measure(
  ...
  facebook::react::TraceSection s("#RNOH::TextMeasurer::measure");
  auto attributedString = facebook::react::ensurePlaceholderIfEmpty_DO_NOT_USE(
  ...
  auto measurement = textStorage->arkUITypography.getMeasurement();
  return measurement;
}
...
auto TextMeasurer::findFitFontSize(
  ...
  if (!hasFit) {
    fittedAttributedString = attributedString;
    ...
    finalTypography = ArkUITypography(
        styledString.get(),
        styledString.m_attachmentCount,
        styledString.m_fragmentLengths,
        layoutConstraints,
...
void TextMeasurer::setTextStorage(const TextStorage::Shared textStorage) const {
  m_textStorageCache.get(
      ...
      [textStorage]() { return textStorage; });
}
...
TextMeasurer::TextStorage::Shared TextMeasurer::getTextStorage(
  ...
  for (auto error : errors) {
    ...
    cacheKey.ceiledWidth += error;
    auto cachedStorage = m_textStorageCache.get(cacheKey);
    if (cachedStorage) {
      auto storage = cachedStorage.value();
      if (storage) {
        return storage;
      }
    }
